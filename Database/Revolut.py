...
def get_market_checklist(bm_prices_full_tuple):
    ...
    try:
        ...
        sp500_tickers = get_revolut_list()
...
with st.sidebar:
    ...
    st.title("RS Momentum (Revolut) v1.0")
    ...
    bt_univ_choice = st.radio("Backtest Universe", ["Watchlist","Revolut Index"])
...
with t4:
    ...
    if st.button("🧪 Execute Advanced Backtest"):
        with st.spinner("Processing History..."):
            ticks_bt = portfolio_list if bt_univ_choice=="Watchlist" else get_revolut_list()
